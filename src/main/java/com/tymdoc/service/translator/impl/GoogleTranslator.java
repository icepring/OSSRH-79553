...
public class GoogleTranslator extends AbstractTranslator {
    ...
    private static final String EN2JA_URL = "http://translate.google.cn/translate_a/single?client=gtx&sl=en&tl=ja&dt=t&q=%s";
    ...
    @Override
    public String translateEn2Ja(String text) {
        try {
            JsonNode response = JsonUtil.fromJson(HttpUtil.get(String.format(EN2JA_URL, HttpUtil.encode(text))));
            return Objects.requireNonNull(response).get(0).get(0).get(0).textValue();
        } catch (Exception e) {
            LOGGER.error("请求谷歌翻译接口异常：请检查本地网络是否可连接外网，也有可能已经被谷歌限流", e);
            return StringUtils.EMPTY;
